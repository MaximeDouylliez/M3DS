...
void GLApplication::updateCamera() {
  ...
  switch (_cameraMode) {
    ...
    case Camera_Follow_Car: {
      ...
      _camera.position(_car.position()+_car.orientation()*Vector3(10,5,0));
           _camera.orientation(_car.orientation());
           _camera.rotate(90, Vector3(0,1,0));

      }
      break;
    case Camera_Follow_Plane: {

      _camera.position(_airplane.position()+_airplane.orientation()*Vector3(0,0,-5));

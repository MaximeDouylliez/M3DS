...
Vector3 Raytrace::computeRayColor(const Ray &ray, int level, double contribution) {
  ...
  */
    ...
    if (inter!=NULL&&level>0&&contribution>0.001) { // existe-t-il une intersection avec la scène ?
        color=computeLocalColor(*inter); // calcul de la couleur par Phong

        double coefReflection=inter->material().reflectionCoefficient();
        double coefRefraction=inter->material().refractionCoefficient();

        if(coefReflection>0){
            Ray reflection=inter->computeReflectRay();
            Ray refraction=inter->computeRefractRay();
            Vector3  colorReflect=computeRayColor(reflection,level-1,coefReflection*contribution);
            Vector3 colorRefract=computeRayColor(refraction,level-1,coefRefraction*contribution);
            color=(1-coefReflection-coefRefraction)*color + coefReflection*colorReflect + coefRefraction*colorRefract;
        }
        delete inter;
    }
    return color;
...
Vector3 Raytrace::computeLocalColor(const Intersection &intersection) {
  ...
  */
    ...
    for (int i = 0; i< nbLight; i++) {
        ...
        float diffuse = max(N.dot(L),0.0);
        double shin = m.shininess();
        double coef = max(V.dot(R),0.0);
        double specular = pow(coef,shin);

        if (inter!=NULL) {
            double lambda = inter->lambda();
            if (lambda > 1)
                result += diffuse*m.diffuse() + specular*m.specular();
            delete inter;
        } else
            result += diffuse*m.diffuse() + specular*m.specular();
    }

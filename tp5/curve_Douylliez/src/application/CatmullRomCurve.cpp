...
intermediate(i,0,point(i)+(parallele/2)*k);
    ...
    intermediate(nbPoint()-2,1,0.5*point(nbPoint()-1)+0.5*intermediate(nbPoint()-2,0));//pour le dernier point intermediaire
    intermediate(0,0,0.5*point(0)+0.5*intermediate(0,1));//pour le premier point intermediaire
// 0.4 et 0.6 comme coef de intermediate donnent des résultats... originaux !
}

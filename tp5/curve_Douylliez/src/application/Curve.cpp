...
int size=castel.size();
        for (int i=0;i<size-1;i++){
            for (int j=0;j<size-i;j++){
                castel[j]=(castel[j+1]*t)+(castel[j]*(1-t));
            }
        }
...
Vector3 Curve::evalCubicVelocity(double t) {
    ...
    res=3*t*t*q[0]+2*t*q[1]+q[2];// t puissance n= n*t puissance n-1 pour n>1 sinon t=1. si t=1 déja, alors t=0;
...
Vector3 Curve::evalCubicAcceleration(double t) {
    ...
    res=6*t*q[0]+2*q[1];// t puissance n= n*t puissance n-1 pour n>1 sinon t=1. si t=1 déja, alors t=0;
